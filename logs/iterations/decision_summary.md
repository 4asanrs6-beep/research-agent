...
## T4: オプション→先物→個別株の波及構造と先回り ← 現在のテーマ

日経オプションの売買が先物を動かし、先物が個別株を動かすという波及経路がある。この構造を理解し、オプション/先物の動きを早期に察知して個別株の売買に活かす。

**問題意識:** 「なんでこんなに株が強いんだろう」と感じるような動きの裏に、オプション→先物→個別株の力学がある
**最終目標:** 個別株の売買判断に使えるシグナルの構築
**データ制約:** J-Quantsでは日中足が取得不可。yfinanceでは日経先物(NKD=F)・個別株(*.T)の1時間足が取得可能

**フェーズ:** Round 1 全4問完了。次ラウンド（オプション建玉データ取得→因果検証）待ち

## T4 知見（Round 1）
| # | 知見 |
|---|------|
| K38 | 1h足リードなし。裁定が効率的で即時追随 |
| K39 | 低βに負のリード(p=0.007)。リバランス売りの可能性 |
| K40 | 夜間先物→ギャップR2=0.35。先物1%→ギャップ3.1%の増幅 |
| K41 | ギャップ後の日中追随率40%。リバーサル傾向 |
| K42 | **出来高1.5倍超日のみ前場→後場リードあり(p=0.04)**。次ラウンドの鍵 |
| K43 | ΔVIX→翌日coef=-0.12(p=0.0000)。五分位完全単調。ただしVIX代理 |

### T4 利用可能データ（予備確認済み）
| データ | ソース | 粒度 | 備考 |
|--------|--------|------|------|
| 日経先物(NKD=F) | yfinance | 1時間足 | 取得確認済み |
| 個別株(*.T) | yfinance | 1時間足 | トヨタ等で確認済み |
| S&P500先物(ES=F) | yfinance | 1時間足 | 取得確認済み |
| USDJPY(JPY=X) | yfinance | 1時間足 | 取得確認済み |
| 日経ETF(1321.T) | yfinance | 1時間足 | 取得確認済み |
| 日経VI(2035.T) | yfinance | 取得失敗 | delistedの可能性 |
| オプション建玉/出来高 | 未確認 | -- | JPXサイトから手動取得の可能性 |
| J-Quants個別株 | J-Quants | 日足のみ | 日中足は現プランで取得不可 |

---

## T3: 2026年3月の日本株ロングショート敗因分析 ← 完了

## T2: turnoverショック固有効果の戦略化 ← 完了
...
| # | 教訓 |
|---|------|
| L1 | 3月の銘柄間リターン差はTOPIXβで説明(R2=0.557)。「モメンタム粛清」は「高β粛清」 |
| L2 | 2月リターンとβの相関が「モメンタム反転」の錯覚を生んだ |
| L3 | 原油は銘柄レベルで説明力なし(R2=0.006) |
| L4 | 危機時にβが急騰(0.32→1.86)。定常βヘッジが劣化しうる |
| L5 | 為替が統計的最強説明変数(p=0.0000)。ただし因果は未確定(共変動) |
| L6 | 原油は市場超過にも直接説明力なし(p=0.601) |
| L7 | モメンタムクラッシュではなくβ主導の一律下落(KS p=0.263) |
| L8 | K37は慢性じわじわ型にはトリガーが出ない。週次拡張が改善余地 |
| L9 | **IWF-IWD相関が1月0.45→2月0.73→3月0.87。2月0.73は警戒水準** |
| L10 | 金利×スプレッド相関が符号反転(+0.35→-0.29, p=0.0506)。リスクオフが全支配 |
| L11 | **VIX閾値ルールは有害。VIX>20で-122%悪化（2月利益取りこぼし）** |
| L12 | LS負転(1/21)がVIX上昇(2/12)より先。VIXはLSの先行指標ではない |
| L13 | モメンタムLS全期間+7.9%。問題はβミスマッチと突然性 |

**統合的結論:** βエクスポージャーの週次監視が最も有効な再発防止策。VIX/モメンタム閾値ルールは有害。唯一の有効な早期シグナルはグロース-バリュー相関の上昇(L9)
...
### ✅ T3-Q03 rate-regime-shift-impact — 金利レジーム転換（検出力不足、方向性明確）

**結論:** insufficient_power。IWF-IWD相関0.45→0.73→0.87でリスクオフ一体化に接近。金利×スプレッド符号反転(p=0.0506)
**得られた教訓:** L9-L10
**詳細:** `logs/experiments/T3-Q03_rate-regime-shift-impact_review.md`
...
### ✅ T3-Q01 vix-creep-early-signal — VIX閾値は有害

**結論:** VIX>20でリスク縮小→2月利益取りこぼしで-122%悪化。地政学イベントはVIXでは検知不能
**得られた教訓:** L11-L13
**詳細:** `logs/experiments/T3-Q01_vix-creep-early-signal_review.md`
...
### ✅ T4-Q01 futures-stock-lead-lag -- NO_LEAD
**結論:** 1h足リードなし(p=0.58)。同時刻R2=0.96。裁定が効率的
**知見:** K38-K39
Phase 5 議論 ......... 済

### ✅ T4-Q02 overnight-gap-futures-signal -- MODERATE_SIGNAL
**結論:** ギャップR2=0.35(p=0.0000)。日中追随率40%(リバーサル)
**知見:** K40-K41
Phase 5 議論 ......... 済

### ✅ T4-Q03 intraday-futures-momentum-spillover-timing -- 出来高急増日のみ有意
**結論:** 全体p=0.74。出来高1.5倍超日のみ交互作用p=0.04
**知見:** K42
Phase 5 議論 ......... 済

### ✅ T4-Q04 put-call-ratio-directional-signal -- VIX代理で強い予測力
**結論:** ΔVIX→翌日p=0.0000、五分位完全単調。ただしPCR取得不可でVIX代理
**知見:** K43
Phase 5 議論 ......... 済

---

### [2026-04-13] T4 Round 1 全4問完了
Q01: NO_LEAD(1hリードなし)。Q02: MODERATE(ギャップR2=0.35、日中リバーサル)。
Q03: 出来高急増日のみリード有意(p=0.04)。Q04: ΔVIX→翌日p=0.0000(VIX代理)。
K38-K43記録。**次: Round 2（オプション建玉データ取得→因果検証）**

### [2026-04-12] T4 Phase 2 再選定
