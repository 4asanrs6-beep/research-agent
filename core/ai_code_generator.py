...
## コード生成ルール（重要: Python 3.11互換）

**実行環境は Python 3.11 です。以下の構文は使用禁止:**
- f-string内でのネストしたクォート: f'{{d['key']}}' は禁止 → 代わりに変数に一度取り出す: val = d['key'] してから f'{{val}}'
- `type` 文（3.12+）
- `except*`（3.11+、ExceptionGroupのみ）
...
**重要: 実行環境は Python 3.11 です。f-string内でのネストしたクォート（例: f'{{d['key']}}'）は構文エラーになります。辞書アクセスは変数に取り出してからf-stringに渡してください。**
